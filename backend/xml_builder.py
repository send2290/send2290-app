...
def build_2290_xml(data: dict) -> str:
    ...
    # Use your IRS-approved EIN from environment
    filer_ein = os.getenv("IRS_EIN", "").replace("-", "").zfill(9)
    if not filer_ein:
        raise ValueError("IRS_EIN environment variable is not set")
    ...
    # Taxpayer EIN (from user input)
    ET.SubElement(hdr, "EIN").text = data.get("ein", "").replace("-", "").zfill(9)
